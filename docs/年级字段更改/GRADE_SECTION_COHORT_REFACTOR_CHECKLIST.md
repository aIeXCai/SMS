...
## 阶段 C：历史数据回填迁移 ✅ FINISHED

### Student 回填方案（方案A：重新导入）

- [x] 导出现有学生数据（包含当前所有学生信息）
- [x] 用户手动补充 section 列（学段：初中部/高中部）和 cohort_year 列（入学年份）
- [x] 修改学生导入模板 Excel，新增 section 和 cohort_year 列
- [x] 按新模板重新导入学生数据
- [x] 生成导入校验报告
...
### Class 数据填充规则 ✅
- [x] 班级是固定的，同一班级和年级不会存在不同届学生
- [x] 根据班级内学生的 cohort_year 填充（正常情况下全班一致）
- [x] 如发现同一班级内存在不同 cohort_year 的学生，**提示管理员手动确认**

**完成情况**：
- 总学生数：1494人（初三799 + 初二695）
- 总班级数：39个（有学生的班级36个）
- cohort 字段：全部填充，无NULL/空值

### Exam 数据填充规则 ⏳
- [ ] 需用户在系统内手动创建考试（选择适用的 cohort 年级格式）

**说明**：Exam 不需要导入模板，用户在系统内手动创建考试时，选择"适用年级"（cohort格式如"初中2024级"）。
...
- [x] 统一 managed_grade 表达方式（managed_section + managed_cohort_year）✅
  - `CustomUser` 新增 `managed_section`（学段）和 `managed_cohort_year`（入学年份）字段
  - `managed_grade` 保留兼容
  - 迁移 `0003_add_managed_section_cohort_year` 已执行
- [ ] 权限判断改造为按新字段做数据范围过滤（暂不启用，由用户决定）
- [x] 角色管理脚本和测试账号生成脚本同步升级 ✅
  - `set_user_role.py`：支持 `--section` + `--cohort-year` 新参数
  - `create_demo_users.py`：示例用户改用新字段
  - `admin.py`：用户管理界面新增新字段

## 阶段 G：测试与验收

- [x] 模型迁移测试（空库、已有数据、脏数据三类）✅
  - 空库迁移: 17张表创建成功
  - 脏数据（NULL cohort空班级17个）: 迁移无报错
- [x] API 兼容测试（旧参数、新参数、混合参数）✅
  - 学生cohort过滤: 695条 ✅
  - 学生状态过滤: 695条 ✅
  - 混合过滤: 695条 ✅
  - stats cohort_choices: 16个选项正常 ✅
- [x] 排名一致性测试（改造前后同数据同排序）✅
  - 4705条成绩全部有年级排名
- [x] 分析页面回归测试（单班、多班、年级分析）✅
  - 班级cohort过滤(初中2024级): 17个班全部匹配 ✅
- [x] 导入导出往返测试（导入后导出字段一致）✅
  - 695学生+17班级+4705成绩全部FK关联正确
- [x] 权限回归（admin/grade_manager/staff）✅
  - admin读写学生/成绩/班级: 200 OK
...
- M2：后端双栈兼容上线
- M3：前端全面切换
- M4：权限治理完成（字段结构已完成，权限数据过滤暂未启用）✅


## 10. 你可以直接照着执行的顺序

- [x] 第 1 天：阶段 A + 阶段 B（字段新增）✅
- [x] 第 2 天：阶段 C（数据回填 + 校验报告）✅
- [x] 第 3-4 天：阶段 D（API/任务/分析服务兼容）✅
- [x] 第 5-6 天：阶段 E（前端切换）✅
- [x] 第 7 天：阶段 F - 模型/Serializer/AuthContext/脚本 ✅
  - F1（模型字段）、F2（Serializer）、F4（AuthContext）、F5（脚本）已完成
  - F3（权限数据过滤）暂不启用
- [x] 第 7 天（续）：阶段 G（测试与验收）✅
