...
from ..models import (
    Student, Class,
    STATUS_CHOICES, CLASS_NAME_CHOICES, GRADE_LEVEL_CHOICES,
...
def student_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('students_grades:student_list')
        else:
...
def student_update_status(request, pk):
    ...
    if new_status and new_status in valid_statuses:
        ...
        # 如果狀態是「毕业」，自動設定畢業日期 (根據 PRD 3.1.7 預留的邏輯，這裡可以做簡易處理)
        if new_status == '毕业' and not student.graduation_date:
    ...
    return redirect('students_grades:student_list') # 更新後導回學生列表頁面
...
def student_batch_import(request):
    if request.method == 'POST':
        ...
        return render(request, 'students/student_batch_import.html', {
            'form': form,
            'title': '批量导入学生',
            'download_template_url': reverse('students_grades:download_student_import_template')
        })
    ...
    if form.is_valid():
        ...
        try:
            with transaction.atomic(): # 確保更新操作的原子性
                ...
                # 如果新狀態是「毕业」，自動設定畢業日期
                if new_status == '毕业':
                    # 只更新狀態不是「毕业」的學生，並設定畢業日期
                    updated_count = students_to_update.exclude(status='毕业').update(status=new_status, graduation_date=timezone.now().date())
                    # 對於已經是毕业狀態的，只更新狀態（雖然這裡實際不會再更新）
...
def student_batch_promote_grade(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            from_grade = form.cleaned_data['current_grade_level']
            to_grade = form.cleaned_data['target_grade_level']
            auto_create_classes = form.cleaned_data.get('auto_create_classes', False)
            ...
            with transaction.atomic():
                ...
                if from_grade:
                    students_to_promote = students_to_promote.filter(current_class__grade_level=from_grade)
                    ...
                    try:
                        ...
                        if current_class:
                            ...
                            if not target_class_obj and auto_create_classes:
                                ...
                                messages.info(request, f"已自动创建班级：{to_grade}{current_class.class_name}。")
                            elif not target_class_obj and not auto_create_classes:
                                errors.append(f"学生 {student.name} (学号: {student.student_id}) 所在的班级 '{current_class.class_name}' 在目标年级 '{to_grade}' 中不存在，且未启用自动创建。跳过升级。")
                                continue # 跳過此學生

                            if target_class_obj:
                                student.current_class = target_class_obj
                                student.grade_level = to_grade  # 同时更新学生的年级字段
                                student.save()
                                updated_count += 1
                        else:
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({
                    'success': True,
                    'message': f"成功将 {updated_count} 名学生升入 {to_grade}。",
                    'updated_count': updated_count,
                    'errors': errors
                })
            else:
                if updated_count > 0:
                    messages.success(request, f"成功将 {updated_count} 名学生升入 {to_grade}。")
                ...
                return redirect('students_grades:student_list') # 升級後導回學生列表頁面
        else:
            # 表單驗證失敗
            # 检查是否为AJAX请求
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
...
def student_batch_graduate(request):
    ...
    try:
        with transaction.atomic(): # 使用事務，確保操作的原子性
            ...
            # 將這些學生的狀態更新為 '毕业'，並設定畢業日期為當前日期
            # 我們只更新那些狀態不是 '毕业' 的學生，以避免不必要的資料庫寫入
            updated_count = students_to_graduate.exclude(status='毕业').update(
                status='毕业',
            ...
            already_graduated_count = students_to_graduate.filter(status='毕业').count()

            message = f"成功將 {updated_count} 名學生設置為『毕业』狀態。"
            if already_graduated_count > 0:
                message += f" 其中有 {already_graduated_count} 名學生已是『毕业』狀態，無需重複操作。"
