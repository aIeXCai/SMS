...
export function AIMessageBubble({
  ...
  // DEBUG: log ambiguous messages
  if (!isUser && (message.status === "ambiguous" || (message.candidates && message.candidates.length > 0))) {
    console.log("[AIMessageBubble] Ambiguous message:", {
      status: message.status,
      candidatesCount: message.candidates?.length ?? 0,
      candidates: message.candidates,
      showDisambiguationCard: !!(message.candidates && message.candidates.length > 0 &&
        (message.status === "ambiguous" || message.status === undefined)),
    });
  }
      ...
      <div className="flex w-full mb-4 justify-start">
        <div className="max-w-[80%] px-4 py-2.5 rounded-2xl rounded-bl-md bg-gray-100 text-gray-400 text-sm">
          <span className="inline-flex items-center gap-1.5">
            <span
              className="w-1.5 h-1.5 rounded-full bg-[#01876c] animate-bounce"
  ...
  // --- Assistant message — status-aware rendering ---

  const hasCandidates =
    message.candidates && message.candidates.length > 0;
  const isAmbiguousWithCandidates =
    hasCandidates &&
    (message.status === "ambiguous" ||
      message.status === undefined);
  // For ambiguous status WITH candidates, use disambiguation card (no badge wrapper)
  // For ambiguous status WITHOUT candidates, show badge wrapper
  const showDisambiguationCard = isAmbiguousWithCandidates;

  // Determine if we need a special status wrapper
  const isSpecialStatus =
    message.status &&
    message.status !== "success" &&
    !showDisambiguationCard; // disambiguation card replaces badge wrapper

  const style = message.status ? STATUS_STYLES[message.status] : undefined;

  // Whether to show the raw data expand/collapse for success or insufficient_data
  const showRawDataToggle =
    message.status === "success" && message.data != null;
  const hasInsufficientData =
    message.status === "insufficient_data" && message.data != null;

  // Subject tags
  const availableSubjects = message.data?.available_subjects as
      ...
      <div
        className={`max-w-[80%] px-4 py-2.5 rounded-2xl rounded-bl-md text-sm
          ...
          ${isSpecialStatus && style
            ? `${style.wrapper} border`
            : "bg-gray-100 text-gray-800"
          }`}
      >
        {/* Header: icon + badge for special statuses */}
        {isSpecialStatus && style && (
          <div className="flex items-center gap-2 mb-2">
            {style.icon}
            <span
              className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${style.badge}`}
            >
              {style.badgeText}
            </span>
          </div>
        ...
        {showDisambiguationCard && (
          <AIDisambiguationCard
            candidates={message.candidates as CandidateExam[]}
            onSelect={(examId) => onDisambiguationSelect?.(examId)}
          />
        )}

        {/* Subject tag chips for subject_not_found */}
        {availableSubjects &&
              ...
              <div className="flex flex-wrap gap-1.5">
                {availableSubjects.map((s) => (
                  <button
                    key={s.subject}
                    type="button"
                    onClick={() => onSubjectTagClick(s.subject)}
                    className="inline-flex items-center px-2 py-0.5 text-xs
                               bg-white border border-yellow-300 rounded-full
        ...
        {/* Raw data expand/collapse for success data */}
        {showRawDataToggle && (
        ...
        {/* Data source annotation for all success responses */}
        {message.status === "success" && (
