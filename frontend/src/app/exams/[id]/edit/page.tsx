...
export default function EditExamPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${backendBaseUrl}/api/exams/options/`, { headers: { ...authHeader } }).then((r) => r.json()),
      fetch(`${backendBaseUrl}/api/exams/${examId}/`, { headers: { ...authHeader } }).then((r) => r.json()),
    ])
      .then(([opts, exam]) => {
        setOptions(opts);
        ...
        if (exam.exam_subjects && exam.exam_subjects.length > 0) {
          setSubjects(
            exam.exam_subjects.map((s: { subject_code: string; max_score: number }) => ({
  ...
  }, [token, examId, authHeader]);

  // Load all subject options whenever grade changes (for the select dropdown)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${backendBaseUrl}/api/exams/${examId}/`, {
        method: "PATCH",
        headers: {
        ...
        <div className="container-fluid">
          <div className="row align-items-center">
            <div className="col-md-8">
              <h1>
                <i className="fas fa-edit me-3"></i>编辑考试
              </h1>
            ...
            <div className="card-body p-4">
              ...
              <div className="d-flex justify-content-between">
                ...
                <button className="btn btn-primary px-5" onClick={handleNextStep}>
                  下一步：科目配置 <i className="fas fa-arrow-right ms-2"></i>
                </button>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="card border-0 shadow-sm" style={{ borderRadius: 15 }}>
              <div className="card-body p-4">
                ...
                <div className="d-flex justify-content-between">
                  <div>
                    <button className="btn btn-outline-secondary px-4 me-2" onClick={() => setStep(1)}>
                  ...
                  >
                    {submitting ? (
                      ...
                      <>
                        <i className="fas fa-save me-2"></i>保存修改
                      </>
                    )}
                  </button>
                </div>
